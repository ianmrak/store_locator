from mock_maps_client import MockUserAgentDistanceSuccess, MockUserAgentGeoCodeSuccess, MockUserAgentFailure, MockUserAgentInvalid
...
class GoogleClientTest(unittest.TestCase):
    ...
    def test_gets_lat_lng_from_geocode_result(self):
        client = GoogleClient(MockUserAgentGeoCodeSuccess(), KEY)
    ...
    def test_exits_get_coords_when_response_fails(self):
        client = GoogleClient(MockUserAgentFailure(), KEY)
        with self.assertRaises(SystemExit) as context:
            client.get_coords(ORIGIN)
    
    def test_exits_get_coords_when_response_invalid(self):
        client = GoogleClient(MockUserAgentInvalid(), KEY)
        with self.assertRaises(SystemExit) as context:
            client.get_coords(ORIGIN)

    def test_gets_distance_from_matrix_result(self):
        client = GoogleClient(MockUserAgentDistanceSuccess(), KEY)
